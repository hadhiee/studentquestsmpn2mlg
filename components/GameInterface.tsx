...
export const GameInterface: React.FC<GameInterfaceProps> = ({ levelId, playerStats, competitors, onUpdateStats, onGameOver, onExit }) => {
  ...
  return (
    <div className="min-h-screen bg-void-dark text-white flex flex-col relative overflow-hidden">
      {/* HUD Header */}
      <header className="bg-slate-900/90 border-b border-slate-700 p-4 sticky top-0 z-40 flex justify-between items-center shadow-lg backdrop-blur-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="text-xs text-slate-400 font-sci-fi">ENERGI TEMPORAL</span>
            <div className="flex gap-1">
              {[...Array(MAX_ENERGY)].map((_, i) => (
                <div key={i} className={`h-2 w-6 rounded-full ${i < energy ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]' : 'bg-slate-700'}`} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col items-end">
            <span className="text-xs text-slate-400 font-sci-fi uppercase">SINKRONISASI</span>
            ...
            <span className="text-xs font-mono text-gold-dust mt-1">{currentScore} PTS</span>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 container mx-auto px-4 py-6 max-w-6xl flex flex-col gap-6 relative">
        ...
        {viewMode === 'MAP' && (
          <div className="transition-all duration-500 flex-1 flex flex-col relative">
              <h2 className="text-holo-blue font-sci-fi text-lg mb-2 flex items-center gap-2">
                  <span className="w-3 h-3 bg-holo-blue rounded-full animate-pulse"></span>
                 ...
                 {loading ? (
                    ...
                    <div className="max-w-3xl mx-auto">
                        <div className="flex justify-between items-center mb-6">
                             <span className="bg-slate-800 text-gold-dust px-3 py-1 rounded text-[10px] font-bold border border-slate-600 uppercase tracking-widest">
                                LOKASI: {LEVEL_1_MAP_NODES[currentNodeIndex].label}
                             </span>
                             <button onClick={() => setViewMode('MAP')} className="text-xs text-slate-400 hover:text-white font-bold">KEMBALI KE PETA</button>
                        </div>

                        <h3 className="text-2xl font-bold font-content mb-8 leading-relaxed text-center">
                            ...
                            {currentQuestion.options.map((option, idx) => {
                                ...
                                return (
                                    <button key={idx} onClick={() => handleAnswer(idx)} disabled={selectedOption !== null} className={btnClass}>
                                        <div className="flex items-center gap-3">
                                            <span className="w-8 h-8 rounded-full bg-slate-900 flex items-center justify-center text-xs font-mono border border-slate-600 group-hover:border-holo-blue transition-colors">
                                                {String.fromCharCode(65 + idx)}
                                            </span>
                                            <span className="font-content text-lg">{option}</span>
                                        </div>
                                    </button>
                                )
                            })}
